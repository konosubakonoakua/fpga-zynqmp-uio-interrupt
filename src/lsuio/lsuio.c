...
static struct option const long_options[] = {
    {"help", no_argument, 0, 'h'},         {"mmap", no_argument, 0, 'm'},
    {"uiodev", required_argument, 0, 'u'}, {"verbose", no_argument, 0, 'v'},
...
static int show_map(struct uio_info_t *info, int map_num) {
  ...
  printf("\tmap[%d]: addr=0x%08X, size=%d", map_num, info->maps[map_num].addr,
...
static void usage(int status) {
  printf("List UIO devices.\n");
  printf("Usage: %s [OPTIONS]\n", program_name);
  printf("Options:\n");
  printf("  -h, --help       display this help and exit\n");
  printf("  -m, --mmap       test if mmap() works for all mappings\n");
  printf("  -v, --verbose    also display device attributes\n");
