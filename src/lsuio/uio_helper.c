...
int uio_get_mem_size(struct uio_info_t *info, int map_num) {
  ...
  sprintf(filename, "/sys/class/uio/uio%d/maps/map%d/size", info->uio_num,
          map_num);
...
int uio_get_mem_addr(struct uio_info_t *info, int map_num) {
  ...
  sprintf(filename, "/sys/class/uio/uio%d/maps/map%d/addr", info->uio_num,
          map_num);
  FILE *file = fopen(filename, "r");
  if (!file)
    return -1;
...
int uio_get_device_attributes(struct uio_info_t *info) {
  ...
  while (n--) {
    sprintf(fullname, "/sys/class/uio/uio%d/device/%s", info->uio_num,
            namelist[n]->d_name);
...
void uio_single_mmap_test(struct uio_info_t *info, int map_num) {
  ...
  if (fd < 0)
    return;
  void *map_addr = mmap(NULL, info->maps[map_num].size, PROT_READ, MAP_SHARED,
